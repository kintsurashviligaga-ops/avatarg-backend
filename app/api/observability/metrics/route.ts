...
import { getDashboardMetrics, getMetricHourSeries } from '@/lib/observability/redisMetrics';
...
export async function GET(req: Request): Promise<Response> {
  ...
  const [dashboard, latencySeries] = await Promise.all([
    getDashboardMetrics(),
    getMetricHourSeries('requests_total', 24),
  ]);
  ...
  return Response.json(
    {
      ...
      dashboard,
      series: {
        requests_total_24h: latencySeries,
      },
